...
例（ルート `mise.toml` の `*-copy` タスクと同じ src / dest を渡す想定）:
...
`ai-agents/skills` と `ai-agents/personal/skills` は同じ配布先へコピーされるため、
それぞれ別に比較する。`.claude/skills/` は配布対象ではないので比較対象にならない。
...
- シンボリックリンク（`codex` の `AGENTS.md` は mise の `codex-link` で `ln -sf`）は copy 対象外なので
  check でも対象外。

## 設計

- 外部依存ゼロ（標準ライブラリのみ）。マッピングは持たず `(mode, src, dest)` を受けるだけで、
  単一情報源はルートの `mise.toml` に保つ。
