...
Mocks for the ports are generated with `go.uber.org/mock` (mockgen), pinned as a `go tool` in `go.mod`. After adding or changing a port, regenerate with `mise run ai-bridge:generate` (`go generate ./...`) and commit the result. Use the generated mocks (`internal/usecase/port/mock`) to unit-test use cases; do not hand-write test doubles for ports.
