...
- `scripts/`: Go apps, one module per directory (`agent-stats`, `ai-bridge`, `config-diff`, `go-verify`, `nvim-sync`, `nvim-usage`, `pipeline-metrics`, `scaffold`). Each has its own `README.md`.
...
### Go apps under `scripts/` (agent-stats, ai-bridge, config-diff, go-verify, nvim-sync, nvim-usage, pipeline-metrics, scaffold)
