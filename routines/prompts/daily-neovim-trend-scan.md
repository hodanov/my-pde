...
1. まず既存 Issue を取得して「提案してはいけない内容」と「最近どの領域に提案が偏っているか」を把握する。以下の 3 つを全て取得する:
   ...
   3. 採用済みの Close 済み Issue: `gh issue list --state closed --label "scan:nvim" --label "adopted" --json number,title,createdAt,closedAt --limit 50`

   1 は「既に提案済み」、2 は「一度不採用になった」提案で、どちらとも重複しない提案を出すこと。3 は重複判定ではなく偏りの判定に使う（手順 3 の【領域ゲート】）。採用済みの提案は設定に取り込まれた結果その「隣」が空くため、放置すると同じ動線の小さな穴を埋め続ける提案に流れやすい。
...
3. このリポジトリで実際に活用できる改善点を「1つだけ」選ぶ。
   ...
   - 【領域ゲート】手順 1 の 3 で取得した採用済み Issue を領域（quickfix / LSP / gitsigns / telescope / 補完 / フォーマッタ / 表示オプション / ターミナル など）ごとに数え、候補と同じ領域に採用済みが何件あるか確認する。3 件以上ある領域は既に十分掘られており、そこへの追加提案は「育った動線の隣に残った小さな穴を埋めるだけ」になって 1 件あたりの効果が逓減する。原則として別の領域から候補を選び直すこと。それでもその領域を選ぶ場合は、課題に「既に採用済みの N 件がありながら、なぜまだ足りないのか」を 1 行で明示する。
   - 具体的な運用上の不便（pain point）を起点にする。「なぜ既存手段では不十分か」は課題に最大 2 点だけ書き、それ以外の代替手段の検討は body に残さない。
   - 各ゲートの確認は起票前に自分で行い、body には結論だけを書く。確認した過程・検討して外した代替手段の網羅列挙を body に入れない。
4. ラベル `scan:nvim` が無ければ `gh label create scan:nvim` で作成。選んだ改善提案を Issue として 1 件だけ起票する: `gh issue create --label "scan:nvim" --title "..." --body "..."`。body には以下を含める:
   ...
   - **変更** — `nvim/config/` の対象ファイルと、そこに入る差分。コード / diff は 20 行以内、コメントは書かない（`.claude/rules/code-comments.md` は Issue のコード例にも適用される）。20 行を超える規模なら、コードを載せず方針と対象ファイルだけ書く。
   - **リスク** — 最大 3 点、各 1 行。
