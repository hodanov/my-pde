...
3. このリポジトリで実際に活用できる改善点を「1つだけ」選ぶ。
   ...
   - 【領域ゲート】手順 1 の 3 で取得した採用済み Issue を領域（quickfix / LSP / gitsigns / telescope / 補完 / フォーマッタ / 表示オプション / ターミナル など）ごとに数え、候補と同じ領域に採用済みが何件あるか確認する。3 件以上ある領域は既に十分掘られており、そこへの追加提案は「育った動線の隣に残った小さな穴を埋めるだけ」になって 1 件あたりの効果が逓減する。原則として別の領域から候補を選び直すこと。それでもその領域を選ぶ場合は、(a) に「既に採用済みの N 件がありながら、なぜまだ足りないのか」を明示する。
