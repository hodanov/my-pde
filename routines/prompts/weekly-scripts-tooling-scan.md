...
3. この PDE で実際に開発効率を高められる提案を「1つだけ」選ぶ。
   ...
   - 【選定ゲート】次のいずれかに該当する候補は起票せず、別の角度を選び直す。過去の不採用（#518 / #545 / #570 / #618 / #646）はすべてこのどれかに当たっている。
     1. 防ぐ失敗が「打ち忘れ・貼り忘れ・更新忘れ」のような単発のヒューマンエラーで、気づけば 1 コマンドで直る。それを事後検出するだけの検査 / 監査 / リンタ系ツールは、常設の保守コストが防げる被害を上回る。
     2. 同じ効果が「既存タスク・既存ツールへの数十行の追記」や「手作業ステップ自体を消す」で得られる。その場合は net-new ではなく、その小さい改善のほうを提案する。
     3. 新規の外部依存や、このリポジトリに実績のない統合方式を前提にする（`scripts/` の Go ツールは標準ライブラリ中心・依存最小が既定）。
     4. オーナーが実際には使っていないツール・端末・CLI を前提にする。
   - 通すべきなのは「オーナーが日常的に繰り返している作業そのものを短くする / 自動化する」道具（採用例: `nvim-sync` / `go-verify` / `config-diff` / `scaffold` / `worktree` / `agent-stats`）。
   - 選定の確認（重複チェック・除外条件・選定ゲート）は起票前に自分で行い、body には結論だけを書く。確認した過程・検討して外した代替手段の網羅列挙を body に入れない。
...
4. ラベル `scan:scripts` が無ければ `gh label create scan:scripts` で作成。選んだ提案を Issue として 1 件だけ起票する: `gh issue create --label "scan:scripts" --title "..." --body "..."`。body には以下を含める:
   - **課題** — どの操作・運用が具体的にどう不便か（どんな開発効率の課題を解くか）。その不便が起きる頻度（毎回の作業か・週何回か）を 1 行で示す。既存手段（既定機能・既存ツール・現行設定）で足りない理由を最大 2 点、各 1〜2 行。
